...
export const BarChart: React.FC<BarChartProps> = ({
  ...
  const TITLE_HEIGHT = Math.round(18 * scale); // Altura do título principal
  const SUBTITLE_HEIGHT = Math.round(14 * scale); // Altura do subtítulo
  const X_AXIS_LABEL_HEIGHT = Math.round(32 * scale); // +32px para labels do eixo X na base
  const Y_AXIS_LABEL_WIDTH = Math.round(90 * scale); // Espaço para labels do eixo Y (ajustado para R$ X.X M)

  const totalHeaderHeight = TITLE_HEIGHT + SUBTITLE_HEIGHT + Math.round(10 * scale); // Espaçamento entre título/subtítulo e chart
  
  const chartWidth = width - 2 * PLOT_AREA_PADDING;
  const chartHeight = height - 2 * PLOT_AREA_PADDING - totalHeaderHeight - X_AXIS_LABEL_HEIGHT;

  const plotAreaX = PLOT_AREA_PADDING + Y_AXIS_LABEL_WIDTH;
  const plotAreaY = PLOT_AREA_PADDING + totalHeaderHeight;
  ...
  // Para bar charts verticais, só nos preocupamos com valores positivos
  ...
  // [TIPOGRAFIA E LABELS]
  const titleFontSize = Math.round(20 * scale); // Ajuste para 20px
  const subtitleFontSize = Math.round(14 * scale); // Novo subtítulo
  ...
  return (
    ...
    >
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
        ...
        <text
          x={width / 2}
          y={PLOT_AREA_PADDING + TITLE_HEIGHT + SUBTITLE_HEIGHT / 2 + Math.round(4 * scale)} // Pequeno espaçamento
          textAnchor="middle"
        ...
        {data.map((item, index) => {
          ...
          // Para este caso, aplicaremos `rx` e `ry` uniformemente, mas o ideal seria uma implementação
          // mais avançada para "apenas no topo" se a referência visual exigir explicitamente.
