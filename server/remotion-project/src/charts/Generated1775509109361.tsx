...
const generateYAxisTicks = (maxValue: number, numTicks: number): number[] => {
  if (maxValue <= 0 || numTicks <= 0) return [0]; // Handle edge cases
    ...
    >
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
        ...
        {subtitle && (
          <text
            x={width / 2}
            y={PLOT_AREA_PADDING + TITLE_HEIGHT + SUBTITLE_HEIGHT / 2 - Math.round(4 * scale)} // Pequeno ajuste vertical
            textAnchor="middle"
            fontSize={subtitleFontSize}
            fontWeight={400}
            fill={'#AAAAAA'} // Cor padrão para subtítulo
          ...
          return (
            <React.Fragment key={`grid-line-${index}`}>
              ...
              <text
                ...
                opacity={gridLineOpacity}
              >
                {formatNumber(tickValue, 'R$ ')} {/* Utiliza R$ como prefixo monetário */}
              </text>
            </React.Fragment>
          );
        })}
        ...
        {data.map((item, index) => {
          ...
          // Conforme regra, para este caso, aplicamos uniformemente.
