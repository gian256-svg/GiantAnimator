...
export const BarChart: React.FC<BarChartProps> = ({
  ...
  const Y_AXIS_LABEL_WIDTH = Math.round(80 * scale); // Espaço reservado para labels do eixo Y
    ...
    >
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
        ...
        {yTickValues.map((tickValue, index) => {
          ...
          return (
            <React.Fragment key={`grid-line-${index}`}>
              ...
              <text
                x={plotAreaX - Math.round(8 * scale)} // 8px de padding à direita do plot area
        ...
        {data.map((value, index) => {
          ...
          // Uma implementação para "apenas no topo" exigiria um `path` mais complexo ou máscaras,
          // que por sua vez aumentaria a complexidade do código e potencial de erro.
          // Para esta entrega inicial, `rx` e `ry` serão aplicados de forma consistente,
          // simulando um arredondamento no topo quando a barra cresce de baixo.
