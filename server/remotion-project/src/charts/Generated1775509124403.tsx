...
// Interface atualizada para corresponder à nova estrutura de dados JSON
...
// Helper para gerar um range de números "arredondados" para os ticks do eixo Y
const generateYAxisTicks = (maxValue: number, numTicks: number): number[] => {
  if (maxValue <= 0) return [0]; // Lida com casos onde o valor máximo é zero ou negativo
  ...
  // Ajusta o 'niceStep' para ser 1, 2 ou 5 vezes o fator de 10
  if (roughStep / factor <= 1.5) {
    niceStep = factor;
  } else if (roughStep / factor <= 3) {
    niceStep = 2 * factor;
  } else if (roughStep / factor <= 7.5) {
    niceStep = 5 * factor;
  } else {
    niceStep = 10 * factor;
  }
  ...
  const ticks = [];
  // Garante que o valor máximo do tick seja pelo menos o maxValue
  const roundedMaxValue = Math.ceil(maxValue / niceStep) * niceStep;

  for (let i = 0; i <= roundedMaxValue; i += niceStep) {
    ticks.push(i);
  }
  // Garante que 0 esteja sempre incluído, se não estiver
  if (ticks[0] !== 0) {
    ticks.unshift(0);
  }
  return ticks;
...
export const BarChart: React.FC<BarChartProps> = ({
  ...
  const TITLE_HEIGHT = Math.round(18 * scale); // Tamanho da fonte do título: 18-22px
  const SUBTITLE_HEIGHT = subtitle ? Math.round(14 * scale) : 0; // Tamanho da fonte do subtítulo: 13-14px
  const SPACE_BETWEEN_TITLE_SUBTITLE = subtitle ? Math.round(4 * scale) : 0; // Espaçamento entre título e subtítulo
  const X_AXIS_LABEL_HEIGHT = Math.round(32 * scale); // +32px para labels do eixo X na base
  const Y_AXIS_LABEL_WIDTH = Math.round(100 * scale); // Espaço para labels do eixo Y (R$ X.X M)

  const chartAreaPaddingTop = PLOT_AREA_PADDING + TITLE_HEIGHT + SUBTITLE_HEIGHT + SPACE_BETWEEN_TITLE_SUBTITLE;
  
  const plotHeight = height - chartAreaPaddingTop - PLOT_AREA_PADDING - X_AXIS_LABEL_HEIGHT;
  const plotWidth = width - (2 * PLOT_AREA_PADDING + Y_AXIS_LABEL_WIDTH);

  const plotAreaX = PLOT_AREA_PADDING + Y_AXIS_LABEL_WIDTH;
  const plotAreaY = chartAreaPaddingTop;
  ...
  // Calcular tick marks do eixo Y e ajustar o valor máximo efetivo para o tick mais alto
  const numYTicks = 5; // Exemplo de 5 ticks principais
  const yTickValues = generateYAxisTicks(rawMaxValue, numYTicks);
  const effectiveMaxValue = yTickValues[yTickValues.length - 1]; // O tick mais alto será o novo valor máximo efetivo
  ...
  const barColor = '#7CB5EC'; // Paleta padrão GiantAnimator - Série 1 (azul)
    ...
    >
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
        ...
        {subtitle && (
          <text
            x={width / 2}
            y={PLOT_AREA_PADDING + TITLE_HEIGHT + SUBTITLE_HEIGHT / 2 + SPACE_BETWEEN_TITLE_SUBTITLE}
            textAnchor="middle"
          ...
          return (
            <React.Fragment key={`grid-line-${index}`}>
              ...
              <text
                ...
                opacity={gridLineOpacity}
              >
                {formatNumber(tickValue, 'R$ ')} {/* Prefixo R$ e formatação para BRL */}
              </text>
            </React.Fragment>
          );
        })}
        ...
        {data.map((item, index) => {
          ...
          // [EDGE CASES E ROBUSTEZ] - Proteger contra NaN para atributos SVG
          ...
          // Em SVGs, rx e ry aplicam-se a todos os cantos do rect.
          // Para um efeito de "apenas topo", seriam necessários paths ou máscaras complexas.
          // Mantendo a simplicidade e aderência ao padrão SVG `rect`, aplicamos uniformemente.
