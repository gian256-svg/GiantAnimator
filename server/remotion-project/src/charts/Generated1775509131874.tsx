...
const formatNumber = (num: number, prefix: string = '', decimals: number = 0): string => {
  ...
  if (isNaN(num)) {
    console.warn(`[${new Date().toISOString()}] GiantAnimator: formatNumber recebeu NaN. Retornando string vazia.`);
    return '';
  }
  ...
  if (absNum < 1000) {
    return `${sign}${prefix}${absNum.toLocaleString('pt-BR', { minimumFractionDigits: decimals, maximumFractionDigits: decimals })}`;
  }
  if (absNum < 1000000) {
    return `${sign}${prefix}${(absNum / 1000).toLocaleString('pt-BR', { maximumFractionDigits: 1 })}k`;
  }
  return `${sign}${prefix}${(absNum / 1000000).toLocaleString('pt-BR', { maximumFractionDigits: 1 })}M`;
};
...
const generateYAxisTicks = (maxValue: number, numTicks: number): number[] => {
  // [EDGE CASES E ROBUSTEZ] - Proteger contra maxValue <= 0
  if (maxValue <= 0 || numTicks <= 0) return [0]; 
...
export const BarChart: React.FC<BarChartProps> = ({
  ...
  const TITLE_TEXT_HEIGHT = Math.round(18 * scale); // Altura estimada para o texto do título (fonte 18-22px)
  const SUBTITLE_TEXT_HEIGHT = subtitle ? Math.round(13 * scale) : 0; // Altura estimada para o texto do subtítulo (fonte 13-14px)
  
  // Margem extra no topo para título: +24px quando título presente [REGRAS DE ESTRUTURA E LAYOUT]
  const TITLE_TOP_MARGIN = title ? Math.round(24 * scale) : 0;
  // Espaçamento entre título e subtítulo
  const TITLE_SUBTITLE_GAP = subtitle && title ? Math.round(4 * scale) : 0; 

  const HEADER_TOTAL_HEIGHT = TITLE_TOP_MARGIN + TITLE_TEXT_HEIGHT + TITLE_SUBTITLE_GAP + SUBTITLE_TEXT_HEIGHT;

  const X_AXIS_LABEL_HEIGHT = Math.round(32 * scale); // +32px para labels do eixo X na base
  const Y_AXIS_LABEL_WIDTH = Math.round(80 * scale); // Espaço para labels do eixo Y (ajustado para R$ X.X M)

  const chartWidth = width - 2 * PLOT_AREA_PADDING;
  const chartHeight = height - 2 * PLOT_AREA_PADDING - HEADER_TOTAL_HEIGHT - X_AXIS_LABEL_HEIGHT;

  const plotAreaX = PLOT_AREA_PADDING + Y_AXIS_LABEL_WIDTH;
  const plotAreaY = PLOT_AREA_PADDING + HEADER_TOTAL_HEIGHT;
  const plotWidth = chartWidth - Y_AXIS_LABEL_WIDTH;
  const plotHeight = chartHeight;

  // [EDGE CASES E ROBUSTEZ] - Verificar dados antes de processar
  if (!Array.isArray(data) || data.length === 0) {
    console.log(`[${new Date().toISOString()}] GiantAnimator: Sem dados para exibir no BarChart. Exibindo fallback.`);
    return (
      <div style={{
  ...
  const barWidthPercentage = 0.65; // 60–70% do espaço, escolhendo 65%
    ...
    >
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
        ...
        {title && (
          <text
            x={width / 2}
            y={PLOT_AREA_PADDING + TITLE_TOP_MARGIN / 2 + TITLE_TEXT_HEIGHT / 2}
            textAnchor="middle"
        ...
        {subtitle && (
          <text
            x={width / 2}
            y={PLOT_AREA_PADDING + TITLE_TOP_MARGIN + TITLE_TEXT_HEIGHT + TITLE_SUBTITLE_GAP + SUBTITLE_TEXT_HEIGHT / 2}
            textAnchor="middle"
            fontSize={subtitleFontSize}
            fontWeight={400}
            fill={subtitleColor}
            opacity={interpolate(frame, [10, 30], [0, 1], { extrapolateRight: 'clamp', config: SPRING_CONFIG_SUBTLE })}
          >
            {subtitle}
          </text>
        )}
        ...
        {yTickValues.map((tickValue, index) => {
          // [EDGE CASES E ROBUSTEZ] - Prevenção de divisão por zero (maxValue já garante >0)
          const y = maxValue > 0 
            ? plotAreaY + plotHeight - (tickValue / maxValue) * plotHeight
            : plotAreaY + plotHeight; // Fallback se maxValue é 0
          
          // [EDGE CASES E ROBUSTEZ] - Proteger contra NaN em Y
        ...
        {data.map((item, index) => {
          ...
          // Se o valor for muito pequeno mas positivo, garantir altura mínima visível [REGRAS DE EDGE CASES E ROBUSTEZ]
          ...
          // [EDGE CASES E ROBUSTEZ] - Proteger contra NaN em X, Y da barra
          const safeBarX = isNaN(barX) ? 0 : barX;
          const safeBarY = isNaN(barY) ? plotAreaY + plotHeight : barY;
          ...
          // Para `rect` em SVG, `rx` e `ry` aplicam-se a todos os cantos.
          // Para simular "apenas no topo" durante o crescimento, um `path` customizado
          // seria ideal, mas mais complexo. Aplicaremos uniformemente `rx`/`ry` para simplicidade.
        ...
        {labels.map((label, index) => {
          ...
          // [EDGE CASES E ROBUSTEZ] - Proteger contra NaN em X, Y do label
