...
const formatNumber = (num: number, prefix: string = '', decimals: number = 0): string => {
  ...
  if (absNum < 1000) {
    return `${sign}${prefix}${absNum.toLocaleString('en-US', { minimumFractionDigits: decimals, maximumFractionDigits: decimals })}`;
  }
  if (absNum < 1000000) {
    return `${sign}${prefix}${(absNum / 1000).toLocaleString('en-US', { maximumFractionDigits: 1 })}k`;
  }
  return `${sign}${prefix}${(absNum / 1000000).toLocaleString('en-US', { maximumFractionDigits: 1 })}M`;
};
...
export const BarChart: React.FC<BarChartProps> = ({
  ...
  const TITLE_HEIGHT = Math.round(18 * scale); // ~18px para o título (apenas altura ocupada pelo texto)
  const SUBTITLE_HEIGHT = subtitle ? Math.round(13 * scale) : 0; // ~13px para o subtítulo
  const HEADER_OFFSET = Math.round(24 * scale); // Margem extra no topo para título/subtítulo
  ...
  const totalHeaderHeight = TITLE_HEIGHT + SUBTITLE_HEIGHT + (subtitle ? Math.round(4 * scale) : 0); // Espaço entre título/subtítulo
  const chartWidth = width - 2 * PLOT_AREA_PADDING;
  const chartHeight = height - 2 * PLOT_AREA_PADDING - totalHeaderHeight - HEADER_OFFSET - X_AXIS_LABEL_HEIGHT;

  const plotAreaX = PLOT_AREA_PADDING + Y_AXIS_LABEL_WIDTH;
  const plotAreaY = PLOT_AREA_PADDING + HEADER_OFFSET + totalHeaderHeight;
  ...
  return (
    ...
    >
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
        ...
        <text
          x={width / 2}
          y={PLOT_AREA_PADDING + TITLE_HEIGHT / 2 + Math.round(8 * scale)} // Ajuste para centralizar visualmente
          textAnchor="middle"
        ...
        {subtitle && (
          <text
            x={width / 2}
            y={PLOT_AREA_PADDING + TITLE_HEIGHT + SUBTITLE_HEIGHT / 2 + Math.round(12 * scale)} // Abaixo do título
            textAnchor="middle"
