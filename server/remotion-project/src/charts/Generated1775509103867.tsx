...
console.log(`[${new Date().toISOString()}] GiantAnimator: Gerando componente Remotion TypeScript para Bar Chart.`);
...
export const BarChart: React.FC<BarChartProps> = ({
  ...
  // const minValue = Math.min(...data, 0); // Não usado para Bar Chart vertical simples
  ...
  const barGapPercentage = 0.35; // 30–40% do espaço total por categoria, escolhendo 35%
  const barWidthPercentage = 1 - barGapPercentage; // 60–70% para a barra, escolhendo 65%
    ...
    >
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
        ...
        {data.map((value, index) => {
          ...
          // Para este caso inicial, aplicaremos `rx` e `ry` uniformemente (4px em todos os cantos),
          // mas o ideal seria uma implementação mais avançada para "apenas no topo" se uma referência
          // visual específica assim o exigir.
          const borderRadius = Math.round(4 * scale);

          // [EDGE CASES E ROBUSTEZ] - Proteção contra NaN
          const safeBarX = isNaN(barX) ? 0 : barX;
          const safeAnimatedY = isNaN(animatedY) ? 0 : animatedY;
          ...
          return (
            <React.Fragment key={labels[index]}>
              ...
              <text
                ...
                y={safeAnimatedY - Math.round(8 * scale) - labelYOffsetAnimation} // +8px da ponta da barra, ajustado pela animação
                textAnchor="middle"
