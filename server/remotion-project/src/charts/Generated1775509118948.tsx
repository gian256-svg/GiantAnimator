...
const formatNumber = (num: number, currencySymbol: string = '', decimals: number = 0): string => {
  ...
  if (absNum < 1000) {
    ...
    formatted = `${(num / 1000).toLocaleString('en-US', { maximumFractionDigits: 1 })}`;
  } else {
    formatted = `${(num / 1000000).toLocaleString('en-US', { maximumFractionDigits: 1 })}`;
  }

  // Adiciona o sinal para números negativos se não for zero
  const sign = num < 0 && absNum > 0 ? '-' : '';
  return `${sign}${currencySymbol}${formatted}${absNum >= 1000 && absNum < 1000000 ? 'k' : absNum >= 1000000 ? 'M' : ''}`;
};
    ...
    >
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
        ...
        {data.map((value, index) => {
          ...
          const baseBarY = plotAreaY + plotHeight - actualBarHeight;
          ...
          const animatedY = interpolate(
            ...
            [plotAreaY + plotHeight, baseBarY], // Y inicial (base) até Y final (topo da barra)
            {
              extrapolateRight: 'clamp',
              config: SPRING_CONFIG_MAIN,
            }
          ...
          // Para aderir estritamente à regra "topo apenas" sem uma referência visual,
          // usamos um path SVG customizado em vez de `rx`/`ry` no `<rect>`.
          const borderRadius = Math.round(4 * scale);
          
          // Path para barra com cantos arredondados apenas no topo.
          // Garante que o raio não exceda a largura/altura disponível.
          const safeRadius = Math.min(borderRadius, barWidth / 2, animatedHeight > 0 ? animatedHeight / 2 : 0);

          const barPath = `
            M ${barX}, ${animatedY + safeRadius}
            Q ${barX}, ${animatedY} ${barX + safeRadius}, ${animatedY}
            H ${barX + barWidth - safeRadius}
            Q ${barX + barWidth}, ${animatedY} ${barX + barWidth}, ${animatedY + safeRadius}
            V ${animatedY + animatedHeight}
            H ${barX}
            Z
          `;

          // [EDGE CASES E ROBUSTEZ] - Proteção para NaN
          const safeBarPath = isNaN(barX) || isNaN(animatedY) || isNaN(barWidth) || isNaN(animatedHeight) ? '' : barPath;
