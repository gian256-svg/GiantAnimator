...
const SPARKLE_COLORS = [
  "rgba(255, 20, 20, 1)",   // Red
  "rgba(255, 100, 50, 1)",  // Orange/Red
...
export const DUOPromoBoard: React.FC = () => {
  ...
  // Card Positioning (Left-aligned cinematic composition)
  const cardTargetX = -600; 
  const cardTargetY = 0;    
  ...
  const tumbleRotY = interpolate(entranceSpring, [0, 1], [40, 0]);
  const tumbleRotZ = interpolate(entranceSpring, [0, 1], [10, 0]);

  const currentX = interpolate(entranceSpring, [0, 1], [0, cardTargetX]);
  const currentY = interpolate(entranceSpring, [0, 1], [100, cardTargetY]);
  ...
  return (
    <AbsoluteFill className="duo-container" style={{ backgroundColor: "#020101" }}>

      {/* --- BACKGROUND GLOW --- */}
      <div className="duo-bg-glow" style={{ 
      ...
      <div style={{ width: "100%", height: "100%", transform: `translate3d(${shakeX}px, ${shakeY}px, ${finalCameraZ}px)`, zIndex: 10 }}>
        ...
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: `translate(${cardTargetX}px, ${cardTargetY}px)` }}>
          {burstOpacity > 0 && (
            <div className="duo-central-burst" style={{ opacity: burstOpacity, transform: `translate(-50%, -50%) scale(${burstScale})` }} />
          )}
          
          {flareOpacity > 0 && (
            <>
              <div className="duo-horizontal-flare" style={{ opacity: flareOpacity * 1.5, transform: `translate(-50%, -50%) scaleX(${flareScaleX})` }} />
              <div className="duo-horizontal-flare" style={{ opacity: flareOpacity * 0.6, transform: `translate(-50%, -50%) rotate(15deg) scaleX(${flareScaleX * 0.7})` }} />
              <div className="duo-horizontal-flare" style={{ opacity: flareOpacity * 0.6, transform: `translate(-50%, -50%) rotate(-15deg) scaleX(${flareScaleX * 0.7})` }} />
            </>
          )}

          {ringOpacity > 0 && (
            <div className="duo-shockwave-ring" style={{ opacity: ringOpacity, transform: `translate(-50%, -50%) scale(${ringScale})` }} />
          )}
        </div>

        {/* --- HARDWARE ACCELERATED PARTICLE SIMULATION --- */}
        <div className="duo-particles-layer">
          {burstParticles.map(p => {
            ...
            return (
              <div key={p.id} className="duo-energy-particle" style={{
                top: 0, left: 0, 
        ...
        <div className="duo-cards-container">
          {frame >= entranceStart - 3 && (
            <div
              className="duo-single-promo-card"
              style={{
                left: `calc(50% - 270px)`, 
                top: `calc(50% - 340px)`,  
                ...
                boxShadow: `0 0 60px rgba(255, 20, 40, ${0.4 * pulseOpacity}), 0 0 20px rgba(255, 20, 40, ${0.8 * pulseOpacity}), 0 40px 80px rgba(0,0,0,0.9)` 
              }}
            >
              ...
              <div style={{
                ...
                boxShadow: `inset 0 0 30px rgba(255, 20, 40, ${0.5 * pulseOpacity})`
              }} />

              <Img 
                src={staticFile(`ArteAd01/Card-7-Duo_Gourmet.png`)} 
